...
const login = async (req, res, next) => {
    debugLog('=== LOGIN FUNCTION STARTED ===');
    const connection = await pool.getConnection();
    
    try {
        const { email, password, ip_address, device_info } = req.body;
        
        debugLog(`Login attempt with body: ${JSON.stringify(req.body)}`);
        
        if (!email) {
            debugLog('Missing email in login request');
            throw new Error('Email is required');
        }
        
        if (!password) {
            debugLog('Missing password in login request');
            throw new Error('Password is required');
        }
        
        await connection.beginTransaction();

        debugLog(`Query params - email: ${email}, password: ${password ? '[PRESENT]' : '[MISSING]'}`);
        ...
        debugLog(`Found ${users.length} users with email: ${email}`);
        
        const user = users[0];
        
        if (!user) {
            debugLog('No user found with this email');
            throw new Error('Invalid email or password');
        }
        
        const passwordMatches = await bcrypt.compare(password, user.password);
        debugLog(`Password match result: ${passwordMatches}`);
        ...
        debugLog('Generated tokens successfully');
        
        debugLog(`Insert token params - user_id: ${user.user_id}, tokens: [PRESENT]`);
        await connection.execute(
        ...
        if (ip_address) {
            debugLog(`Logging IP - user_id: ${user.user_id}, ip: ${ip_address}, device: ${device_info || 'null'}`);
            await connection.execute(
                'INSERT INTO users_ip (user_id, ip_address, login_timestamp, device_info) VALUES (?, ?, NOW(), ?)',
                [user.user_id, ip_address, device_info || null]
            );
        } else {
            debugLog('No IP address provided, skipping IP logging');
        }
        ...
        connection.release();
        debugLog('=== LOGIN FUNCTION COMPLETED ===');
    }
};
