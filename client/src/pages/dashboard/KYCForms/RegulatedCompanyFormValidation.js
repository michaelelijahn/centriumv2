// Exact field mappings extracted from RegulatedCompanyForm validation functions
// Based on the existing validation logic in RegulatedCompanyForm.jsx
...
export const STEP_FIELD_MAPPINGS = {
  ...
  // Step 1 - Email Registration (validateEmailStep lines 132-145)
  1: {
    requiredFields: {
      email: {
        fieldName: 'email',
  ...
  // Step 2 - Company Details (validateCompanyDetailsStep lines 147-196)
  2: {
    requiredFields: {
  ...
  // Step 3 - Document Upload (validateDocumentUploadStep lines 198-219)
  3: {
    requiredFields: {
      '0_0': {
  ...
  // Step 4 - Authorize Person (validateAuthorizePersonStep lines 221-312)
  4: {
    requiredFields: {
    ...
    // Bank accounts validation (lines 280-304)
    bankAccountFields: {
      bankName: {
        fieldName: 'bankName',
        label: 'Bank Name',
  ...
  // Step 5 - Passport Upload (validatePassportUploadStep lines 314-322)
  5: {
    requiredFields: {
      authorizePersonPassport: {
        fieldName: 'uploadedDocuments',
        documentKey: 'authorizePersonPassport',
        label: 'Authorize Person Passport',
        validation: 'file'
      }
    },
    conditionalFields: {}
  ...
  // Step 6 - Document Agreements (validateDocumentAgreementsStep lines 324-358)
  6: {
    requiredFields: {
      companyProfile: {
      ...
      accountOpeningApplication: {
        fieldName: 'accountOpeningApplication',
...
import { validateEmail, validatePhoneWithCountryCode } from '../../../common/helpers';

/**
 * Efficient validation function that uses the field mappings
 * @param {number} stepIndex - The step to validate (0-6)
